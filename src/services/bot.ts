import { GameState, WildcardCard, PropertyCard, ActionCard, GameAction } from '../types/game';
...
export const evaluateBotTurn = (state: GameState, botId: string, style: BotStyle = 'Aggressive'): BotDecision => {
  ...
  const getCommentary = (reasonKey: string, cardName?: string, details?: string): string => {
    ...
    return comments[style][reasonKey] || `Decided to play ${cardName || 'card'}.`;
  };
  ...
  for (const card of hand) {
    ...
    if (card.type === 'Wildcard') {
      ...
      for (const col of wildcard.colors) {
        ...
        const evaluatedSets = restructureProperties([...tempProps, copyWildcard]);
        const setNowComplete = evaluatedSets.find(s => s.color === col)?.isComplete;

        if (setNowComplete) {
          return {
            action: {
              type: 'PLAY_CARD',
              payload: { playerId: botId, cardId: card.id, targetZone: 'properties', options: { color: col } }
            },
    ...
    if (card.type === 'Property') {
      ...
      const existingCount = bot.properties.find(s => s.color === prop.color)?.cards.length || 0;
      if (existingCount > 0 || style === 'Aggressive') {
        return {
          action: {
            type: 'PLAY_CARD',
            payload: { playerId: botId, cardId: card.id, targetZone: 'properties' }
          },
    ...
    if (card.type === 'Action') {
      ...
      if (action.actionType === 'Deal Breaker') {
        ...
        if (playerCompleteSet) {
          return {
            action: {
              type: 'PLAY_CARD',
              payload: { playerId: botId, cardId: card.id, targetZone: 'center', options: { targetColor: playerCompleteSet.color } }
            },
      ...
      if (action.actionType === 'Sly Deal') {
        ...
        const eligibleSets = player.properties.filter(s => !s.isComplete && s.cards.length > 0);
        if (eligibleSets.length > 0) {
          const targetCard = eligibleSets[0].cards[0];
          return {
            action: {
              type: 'PLAY_CARD',
              payload: { playerId: botId, cardId: card.id, targetZone: 'center', options: { targetCardId: targetCard.id } }
            },
            intentReason: getCommentary('play_action_attack', card.name)
          };
        }
      ...
      if (action.actionType === 'Forced Deal') {
        const playerEligibleSets = player.properties.filter(s => !s.isComplete && s.cards.length > 0);
        const botEligibleSets = bot.properties.filter(s => !s.isComplete && s.cards.length > 0);
        if (playerEligibleSets.length > 0 && botEligibleSets.length > 0) {
          ...
          return {
            action: {
              type: 'PLAY_CARD',
              payload: { playerId: botId, cardId: card.id, targetZone: 'center', options: { targetCardId: targetCard.id, swapCardId: swapCard.id } }
            },
            intentReason: getCommentary('play_action_attack', card.name)
          };
        }
      ...
      if (action.actionType === 'Debt Collector' || action.actionType === "Its My Birthday") {
        return {
          action: {
            type: 'PLAY_CARD',
            payload: { playerId: botId, cardId: card.id, targetZone: 'center' }
      ...
      if (action.actionType === 'Rent' && action.rentColors) {
        ...
        if (validRentColors.length > 0) {
          return {
            action: {
              type: 'PLAY_CARD',
              payload: { playerId: botId, cardId: card.id, targetZone: 'center', options: { color: validRentColors[0] } }
            },
            intentReason: getCommentary('play_rent', card.name, validRentColors[0])
          };
        }
      }
  ...
  const remainingProps = hand.filter(c => c.type === 'Property' || c.type === 'Wildcard');
  if (remainingProps.length > 0) {
    const card = remainingProps[0];
    const color = card.type === 'Wildcard' ? (card as WildcardCard).colors[0] : undefined;
    return {
      action: {
        type: 'PLAY_CARD',
        payload: { playerId: botId, cardId: card.id, targetZone: 'properties', options: { color } }
      },
      intentReason: getCommentary('play_property', card.name)
    };
  }
