import { CardColor, PropertySet, PlayerState, WildcardCard, PropertyCard, ActionCard } from '../types/game';
import { PROPERTY_SET_REQS } from './deck';

// Checks if a property set of a specific color is complete
export const isSetComplete = (cards: (PropertyCard | WildcardCard)[], color: CardColor): boolean => {
...
export const calculateRent = (set: PropertySet): number => {
  ...
  const count = set.cards.filter(c => {
    ...
    if (c.type === 'Wildcard') return (c as WildcardCard).currentColor === set.color;
    return false;
  }).length;
...
export const restructureProperties = (cards: (PropertyCard | WildcardCard)[]): PropertySet[] => {
  ...
  cards.forEach(card => {
    ...
    } else if (card.type === 'Wildcard' && (card as WildcardCard).currentColor) {
      setsMap[(card as WildcardCard).currentColor!].push(card);
    }
  });

  const colors: CardColor[] = [
    'Brown', 'Light Blue', 'Pink', 'Orange', 'Red', 'Yellow', 'Green', 'Dark Blue', 'Railroad', 'Utility'
  ];
...
export const findJSNInHand = (player: PlayerState): ActionCard | null => {
  const jsn = player.hand.find(c => c.type === 'Action' && (c as ActionCard).actionType === 'Just Say No');
