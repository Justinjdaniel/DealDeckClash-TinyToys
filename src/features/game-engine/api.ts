...
const advanceTurn = (nextState: GameState, logMsg: (msg: string) => void) => {
  if (!nextState.players || nextState.players.length === 0) return;
  ...
  const handCards = nextPlayer.hand || [];
...
export const canDispatch = (state: GameState, action: GameAction): boolean => {
  ...
  switch (action.type) {
    ...
    case "RESOLVE_PAYMENT": {
      ...
      const callerPlayer = (state.players || []).find(
        (p) => p?.id === callerPlayerId,
      );
      return !!targetPlayer && !!callerPlayer;
    }
...
export const dispatchAction = (
  ...
  switch (action.type) {
    ...
    case "PLAY_CARD": {
      ...
      if (targetZone === "bank") {
        ...
        if (actionCard.actionType === "Pass Go") {
          ...
          const target = (nextState.players || []).find(
            (p) => p?.id !== playerId,
          );
          if (target) {
    ...
    case "RESOLVE_PAYMENT": {
      ...
      const callerPlayer = (nextState.players || []).find(
        (p) => p?.id === callerPlayerId,
      );
      ...
      if (nextState.reactionQueue) {
        ...
        if (validSelectedIds.length > 0) {
          ...
          logMsg(
            `${targetPlayer.name} has no assets to pay ${callerPlayer.name}.`,
          );
        }
      }
      accepted = true;
    ...
    case "RESPOND_TO_ACTION": {
      ...
      const responder = (nextState.players || []).find(
        (p) => p?.id === playerId,
      );
      if (!responder) break;
